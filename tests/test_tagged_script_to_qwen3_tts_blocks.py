...
from script2voice import alignment, cli, config, output, script_parser, subtitles, tts
from script2voice.models import SentenceAudio
...
class TaggedScriptTests(unittest.TestCase):
    ...
    def test_parse_tagged_script_rejects_text_before_first_tag(self):
        with self.assertRaises(ValueError):
            script_parser.parse_tagged_script("没有标签的开头\n[summary]\n内容")
    ...
    def test_safe_stem_preserves_chinese_and_removes_path_chars(self):
        self.assertEqual(output.safe_stem("设置/配置:API"), "设置_配置_API")

    def test_clean_caption_removes_boundary_punctuation(self):
        self.assertEqual(subtitles.clean_caption("，就可以进入单视频对话"), "就可以进入单视频对话")
        self.assertEqual(subtitles.clean_caption("”、“哪里讲到了通道处理？"), "哪里讲到了通道处理")
        self.assertEqual(subtitles.clean_caption("、“老师提到的材质命名规范有哪些？"), "老师提到的材质命名规范有哪些")

    def test_split_sentences_keeps_complete_sentences(self):
        text = "大家好，欢迎来到我的频道。今天介绍 vsummary。"

        self.assertEqual(subtitles.split_sentences(text), ["大家好，欢迎来到我的频道", "今天介绍vsummary"])

    def test_make_local_srt_uses_aligned_times(self):
        sentences = [
            SentenceAudio(text="第一句。", duration=1.5, start=0.4, end=1.9),
            SentenceAudio(text="第二句。", duration=2.5, start=2.2, end=4.7),
        ]
        srt = subtitles.make_local_srt(sentences)
    ...
    def test_offset_srt_times(self):
        ...
        shifted = subtitles.offset_srt_times(srt, 2.5)
    ...
    def test_apply_config_loads_settings_and_preset(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                config.apply_config(args, settings, preset)
            finally:
    ...
    def test_load_preset_reads_toml_by_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            preset = config.load_preset("tutorial", presets_dir)
    ...
    def test_synthesize_text_passes_instruct_to_qwen3(self):
        ...
        audio, sample_rate = tts.synthesize_text(model, "测试文本", args)
    ...
    def test_build_sentence_cues_groups_aligned_tokens_by_original_sentences(self):
        ...
        cues = alignment.build_sentence_cues(items, "大家好。今天？")
